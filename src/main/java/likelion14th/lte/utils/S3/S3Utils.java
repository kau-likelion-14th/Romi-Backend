...
public class S3Utils {
    ...
    public S3Dto uploadBytes(byte[] bytes, String fileName, String contentType){
        ...
        PutObjectRequest reg = PutObjectRequest.builder()
                .bucket(config.getBucket())
                .key(key)
                .contentType(ct)
                .build();
        ...
        try{
            s3Client.putObject(reg,RequestBody.fromBytes(bytes));
            return new S3Dto(getUrl(key), key);
        }catch (SdkClientException e) {
            throw new UtilException(S3_UPLOAD_FAILED, e);
        }catch (Exception e){
    ...
    public void deleteFile(String key) {
        ...
        try{
            DeleteObjectRequest reg = DeleteObjectRequest.builder()
                    .bucket(config.getBucket())
                    .key(key)
                    .build();
            s3Client.deleteObject(reg);
